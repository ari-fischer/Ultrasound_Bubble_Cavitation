function [OH_rxn_int,H2O_rxn_int] = get_rxn_energy(T_profile,Mw_vector,kin_param,T0,thermo_dat_params,...
...
for i=1:length(T_profile)
    [Cp_nd,Cv_nd,Hnd_val,~,~]=thermo_data(T_profile(i)./T0,thermo_dat_params);
    H_nd(i,:)=Hnd_val';
    Cv_d(i,:)=Cv_nd'.*R_ig;
    U_d(i,:)=Hnd_val'.*T0.*R_ig-sum(mol_matrix_profile(i,:))*R_ig*T_profile(i);
...
rxns_t = rates.*n0.*V_profile./(4./3.*pi.*R0.^3).*omega;
OH_rxn_int = zeros(1,length(kin_param{4}));
H2O_rxn_int = zeros(1,length(kin_param{4}));
for i=1:length(kin_param{4})
    OH_rxn_int(i) = trapz(time_profile(ad_switch_idx:end),rxns_t(ad_switch_idx:end,i).*stoic(i,2));
    H2O_rxn_int(i) = trapz(time_profile(ad_switch_idx:end),rxns_t(ad_switch_idx:end,i).*stoic(i,1));
end
